...
- **Updated**
  - Fixed bug where the player custom Ring Color wasn't being applied (🐛 @Mona)

## [2.1.1](https://github.com/ChasarooniZ/PF2e-Reactive-Token-Ring/compare/2.1.0...2.1.1) - Fix Custom Ring Color

- **Updated**
  - Fixed bug where custom Ring Color wasn't being applied (🐛 @nargath)
...
- **New**
  - Added a new `random` option which will select the color randomly (based on tokenID) from a list of colors
  - This is intended to be used to help distinguish between duplicate tokens
...
- `Settings Rework`
  - `Autocolor Submenu`
    ...
    - **Overrides world settings when changed from default (`Default` and `#FFFFFE`)**
